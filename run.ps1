...
param(
    [string]$Port = "8080",
    [string]$InstallDir = "$env:USERPROFILE\main-in-motion"
)
...
if (-not $isAdmin) {
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        Start-Process powershell -ArgumentList "-ExecutionPolicy Bypass -File `"$PSCommandPath`" -Port $Port -InstallDir `"$InstallDir`"" -Verb RunAs
        exit
    } else {
...
if (-not (Test-Path "$InstallDir\Dockerfile")) {
...
docker run -d -p "${Port}:8080" --name main-in-motion --restart unless-stopped main-in-motion 2>&1
if ($LASTEXITCODE -ne 0) {
...
if (-not $localIP) {
    $localIP = (Get-NetIPAddress -AddressFamily IPv4 | Where-Object { $_.InterfaceAlias -notmatch "Loopback" -and $_.IPAddress -notlike "169.*" } | Select-Object -First 1).IPAddress
}
